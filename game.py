...
#Throw connection using mysql-connector
conn =mysql.connector.connect(host="localhost",user="root",passwd="123456",database="Project") 
db=conn.cursor()
db.execute("show tables")
lst=db.fetchall()
#checking if table exist or not and if not exist create one
...
if (table,) in lst:
    ...
    db.execute("create table projectdata(Score varchar(20),HighScore varchar(20))")
    conn.commit()
    db.execute("show tables")
    lst1 =db.fetchall()
    print(lst1)
...
class Main(QtWidgets.QWidget):
    ...
    scoredata=int(cfg.get("Game", "score"))
        
    highscoredata=int(cfg.get("Game", "highscore"))
 
    sql=("INSERT INTO PROJECTDATA(Score, HighScore)" "values(%s, %s)")
             
    val=(scoredata, highscoredata)
    try:
        db.execute(sql, val)
